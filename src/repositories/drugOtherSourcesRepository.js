...
class DrugOtherSourcesRepository {
  constructor(dbPool = pool) {
    this.pool = dbPool;
  }

  async create(data) {
    const fields = ["drug_over_id", "data"];
    ...
    const query = `
      INSERT INTO "drug_other_sources" (${fieldsList})
      VALUES (${placeholders})
      RETURNING *
    `;
  ...
  async findAll(filters = {}) {
    let query = 'SELECT * FROM "drug_other_sources"';
    const values = [];
  ...
  async findByDrugIds(drugIds) {
    ...
    const query = `
      SELECT * FROM "drug_other_sources" 
  ...
  async findById(id) {
    const query = 'SELECT * FROM "drug_other_sources" WHERE id = $1';
  ...
  async findByDrugId(drugId) {
    const query =
      'SELECT * FROM "drug_other_sources" WHERE drug_over_id = $1 ORDER BY id';
  ...
  async delete(id) {
    const query = 'DELETE FROM "drug_other_sources" WHERE id = $1 RETURNING id';
    const result = await this.pool.query(query, [id]);
  ...
  async deleteByDrugId(drugId) {
    const query =
      'DELETE FROM "drug_other_sources" WHERE drug_over_id = $1 RETURNING id';
    const result = await this.pool.query(query, [drugId]);
    return result.rowCount;
  }
}

module.exports = { DrugOtherSourcesRepository };
