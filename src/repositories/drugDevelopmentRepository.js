...
class DrugDevelopmentRepository {
  ...
  async create(data) {
    ...
    const query = `
      INSERT INTO "drug_development" (${fieldsList})
      VALUES (${placeholders})
      RETURNING *
    `;
  ...
  async findAll(filters = {}) {
    let query = 'SELECT * FROM "drug_development"';
    const values = [];
  ...
  async findById(id) {
    const query = 'SELECT * FROM "drug_development" WHERE id = $1';
  ...
  async findByDrugId(drugId) {
    const query =
      'SELECT * FROM "drug_development" WHERE drug_over_id = $1 ORDER BY id';
  ...
  async delete(id) {
    const query = 'DELETE FROM "drug_development" WHERE id = $1 RETURNING id';
    const result = await this.pool.query(query, [id]);
  ...
  async deleteByDrugId(drugId) {
    const query =
      'DELETE FROM "drug_development" WHERE drug_over_id = $1 RETURNING id';
    const result = await this.pool.query(query, [drugId]);
    return result.rowCount;
  }
}

module.exports = { DrugDevelopmentRepository };
