...
class DrugActivityRepository {
  ...
  async create(data) {
    ...
    const query = `
      INSERT INTO "drug_activity" (${fieldsList})
      VALUES (${placeholders})
      RETURNING *
    `;
  ...
  async findAll(filters = {}) {
    let query = 'SELECT * FROM "drug_activity"';
    const values = [];
  ...
  async findById(id) {
    const query = 'SELECT * FROM "drug_activity" WHERE id = $1';
  ...
  async findByDrugId(drugId) {
    const query =
      'SELECT * FROM "drug_activity" WHERE drug_over_id = $1 ORDER BY id';
  ...
  async delete(id) {
    const query = 'DELETE FROM "drug_activity" WHERE id = $1 RETURNING id';
    const result = await this.pool.query(query, [id]);
  ...
  async deleteByDrugId(drugId) {
    const query =
      'DELETE FROM "drug_activity" WHERE drug_over_id = $1 RETURNING id';
    const result = await this.pool.query(query, [drugId]);
    return result.rowCount;
  }
}

module.exports = { DrugActivityRepository };
