...
class DrugLicencesMarketingRepository {
  ...
  async create(data) {
    ...
    const query = `
      INSERT INTO "drug_licences_marketing" (${fieldsList})
      VALUES (${placeholders})
      RETURNING *
    `;
  ...
  async findAll(filters = {}) {
    let query = 'SELECT * FROM "drug_licences_marketing"';
    const values = [];
  ...
  async findByDrugIds(drugIds) {
    ...
    const query = `
      SELECT * FROM "drug_licences_marketing" 
  ...
  async findById(id) {
    const query = 'SELECT * FROM "drug_licences_marketing" WHERE id = $1';
  ...
  async findByDrugId(drugId) {
    const query =
      'SELECT * FROM "drug_licences_marketing" WHERE drug_over_id = $1 ORDER BY id';
  ...
  async update(id, data) {
    ...
    const query = `
      UPDATE "drug_licences_marketing"
      SET ${setClause}
      WHERE id = $1
      RETURNING *
    `;
  ...
  async delete(id) {
    const query =
      'DELETE FROM "drug_licences_marketing" WHERE id = $1 RETURNING id';
    const result = await this.pool.query(query, [id]);
  ...
  async deleteByDrugId(drugId) {
    const query =
      'DELETE FROM "drug_licences_marketing" WHERE drug_over_id = $1 RETURNING id';
    const result = await this.pool.query(query, [drugId]);
    return result.rowCount;
  }
}

module.exports = { DrugLicencesMarketingRepository };
